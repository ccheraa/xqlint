...
exports.JSONParseTreeHandler = function (code) {
    ...
    function popNode() {
        ...
        if (ptr.name === 'EQName' && ptr.value === undefined) {
            var getValue = (prevValue, child) => child.value
                ? prevValue + child.value
                : child.children.reduce(getValue, prevValue);
            if (ptr.children.length > 0) {
                ptr.value = getValue('', ptr.children[0]);
            }
            ptr.children.pop();
        }
        if (ptr.name === 'FunctionDecl') {
            var sequenceTypeIndex = ptr.children.findIndex(child => child.name === 'SequenceType');
            if (sequenceTypeIndex > 0) {
                var prevIndex = sequenceTypeIndex - 1;
                while (prevIndex > 0 && ptr.children[prevIndex].name === 'WS') {
                    prevIndex--;
                }
                if (ptr.children[prevIndex].name === 'TOKEN' && ptr.children[prevIndex].value === 'as') {
                    var returnType = {
                        name: 'ReturnType',
                    };
                    returnType.children = ptr.children.slice(prevIndex, sequenceTypeIndex + 1).map(child => ({
                        ...child,
                        getParent: returnType,
                    }));
                    returnType.pos = {
                        sl: returnType.children[0].pos.sl,
                        sc: returnType.children[0].pos.sc,
                        el: returnType.children[returnType.children.length - 1].pos.el,
                        ec: returnType.children[returnType.children.length - 1].pos.ec,
                    };
                    ptr.children = [...ptr.children.slice(0, prevIndex), returnType, ...ptr.children.slice(sequenceTypeIndex + 1, ptr.children.length)];
                }
            }
        }
